...
class supstar(object):
	...
	def draw(self):
		...
		elif(self.alpha < random.uniform(0.1,0.4)):
		...
		# or
		# current alpha = current alpha + (-0.5 * bacon.timestep)
		self.alpha += (self.alpha_value * bacon.timestep*random.uniform(0.1,2))

		


		bacon.set_color(random.uniform(0.4,0.8),random.uniform(0.2,0.4),random.uniform(0.6,1),self.alpha)
		bacon.fill_rect(self.x - self.width/6, self.y - self.height/2, self.x + self.width/6, self.y + self.height/2)
		bacon.set_color(random.uniform(0.2,0.4),random.uniform(0.4,0.8),random.uniform(0.6,1),self.alpha)
...
class Game(bacon.Game):
	...
	def on_init(self):
		...
		for i in xrange(1,300):
			x = random.randint(0,2*bacon.window.width)
			y = random.randint(0,2*bacon.window.height)
